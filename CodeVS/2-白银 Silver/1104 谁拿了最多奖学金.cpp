/*题目描述:某校的惯例是在每学期的期末考试之后发放奖学金。发放的奖学金共有五种，获取的条件各自不同：
  ...
  输入描述:输入第一行是一个整数N（1<=N<=100），表示学生的总数。
  ...
  输出描述:输出包括共三行，第一行是获得最多奖金的学生的姓名，第二行是这名学生获得的奖金总数。
...
int main()
{
	...
	for(int i=1;i<=n;i++)
    {
    	b[i]=0;
		a[i].datain();
    	if(a[i].qimo>80&&a[i].lunwen>=1)  b[i]+=8000;
    	if(a[i].qimo>85&&a[i].banji>80)  b[i]+=4000;
    	if(a[i].qimo>90)  b[i]+=2000;
    	if(a[i].qimo>85&&a[i].shengfen=="Y")  b[i]+=1000;
    	if(a[i].banji>80&&a[i].ganbu=="Y")  b[i]+=850;
    	sum+=b[i];
        if(b[i]>imax)  imax=b[i],p=i;
    }
    cout<<a[p].name<<endl<<imax<<endl<<sum;
