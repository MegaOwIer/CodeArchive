...
int main() {
    ...
    Polynomial_NTT::poly_t delta = {1, 6, 1};
    delta = Polynomial_NTT::getSqrt(delta, mxlen);
    
    Polynomial_NTT::poly_t A = delta;
    A[0]++, A[1]++;
    for(int &val : A)
        val = val * inv2 % mod;
    A = Polynomial_NTT::getLogarithm(A, mxlen);
    for(int &val : A)
        val = val * static_cast<LL>(n + 1) % mod;
    A = Polynomial_NTT::getExponential(A, mxlen);

    delta = Polynomial_NTT::getInverse(delta, mxlen);

    Polynomial_NTT::poly_t ans = Polynomial_NTT::multiply(A, delta);
    for(int i = 1; i <= k; i++)
        cout << (i <= n ? ans[i] : 0) << ' ';
    return 0;
}
