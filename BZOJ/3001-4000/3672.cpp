...
const int MX=200005;
typedef vector<pair<int,LL> >::iterator Vit;
...
void dfs1(int u)
{
	for(Vit i=G[u].begin();i!=G[u].end();++i)
		dis[i->to]=dis[u]+i->val,dfs1(i->to);
}
void getG(int u,int totsiz,int& O)
{
	siz[u]=1,mxsz[u]=0;
	for(Vit i=G[u].begin();i!=G[u].end();++i)if(!used[i->to])
...
void dfs2(int u)
{
	...
	for(Vit i=G[u].begin();i!=G[u].end();++i)if(!used[i->to])dfs2(i->to);
}
...
void solve(int root,int totsiz)
{
	...
	if(totsiz==1)return;
	getG(root,totsiz,rt);
	for(Vit i=G[rt].begin();i!=G[rt].end();++i)used[i->to]=1;
	solve(root,totsiz-siz[rt]+1);
	cnt=0;
	for(Vit i=G[rt].begin();i!=G[rt].end();++i)dfs2(i->to);
	sort(a+1,a+cnt+1);
	...
	for(Vit i=G[rt].begin();i!=G[rt].end();++i)solve(i->to,siz[i->to]);
}
